...
public class ManageSelfAccountViewImpl extends ViewWithUiHandlers<MyUiHandlers> implements ManageSelfAccountPresenter.MyView, ResizeHandler {
    ...
    @Inject
    ManageSelfAccountViewImpl(final EventBus eventBus, ListDataProvider<AccountEntity> dataProvider) {
	...
	@Override
	public void showCurrentAccount(List<AccountEntity> user) {
		GWT.log("showAccounts");
		int filled = user.size() % celltable.getPageSize();
		if(filled != 0) {
			filled = celltable.getPageSize() - filled;
			for (int looper = 0; looper < filled; looper++)
				user.add(null);
		}
		dataprovider.setList(user);
	}
